...
export async function runBenchmark(fn: typeof stringifyAccessor) {
  ...
  bench
    ...
    .add("Mixed index, property, and symbol access", () => {
      fn<ComplexObject>((o) => o.a[mySymbol]?.b[0].c?.[1]?.d.e.f, {
        stringifySymbols: true,
      });
    })
    .add("Accessing built-in properties", () => {
      fn<Window>((window) => window.document.body.firstChild?.baseURI?.[1]);
    });
